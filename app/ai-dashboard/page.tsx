...
import { 
  Search, Bell,
  Image as ImageIcon, Video, Mic, Sparkles, Layout,
  Type,
  CheckCircle2, ArrowRight,
  Flame, CheckCircle, Target, BookOpen, ExternalLink
} from 'lucide-react';
import Image from 'next/image';
import { useRouter } from 'next/navigation';
import ProfileDropdown from '@/components/ProfileDropdown';

// Tool logo mapping — using publicly available favicon/logo URLs
const TOOL_LOGOS: Record<string, string> = {
  ...
  "Google Nano Banana":     "https://www.google.com/s2/favicons?domain=google.com&sz=64",
  "Microsoft 365":          "https://www.google.com/s2/favicons?domain=microsoft365.com&sz=64",
  ...
  "Hendra":                 "https://www.google.com/s2/favicons?domain=hedra.com&sz=64",
  ...
  "Google Doc Voice Typing":"https://www.google.com/s2/favicons?domain=docs.google.com&sz=64",
...
export default function AIDashboardPage() {
  ...
  const toolMappings: Record<string, string[]> = {
    "Text-Text":        ["ChatGPT", "Gemini", "Perplexity", "Claude AI", "DeepSeek", "Notion", "Quillbot", "Elicit", "Smodin", "Genspark"],
    "Text-Audio":       ["Suno", "MicMonster", "ElevenLabs"],
    "Text-Image":       ["ChatGPT", "Krea", "Ideogram", "Freepik", "Leonardo.AI", "Google Nano Banana", "Microsoft 365"],
    "Text-Video":       ["InVideo", "Krea", "Imagine Art", "Hedra", "Google Veo", "Kapwing", "Kling AI", "Synthesia AI", "HeyGen", "Colossyan", "Hailuo"],
    "Text-Animation":   ["Animaker", "Krea", "Hedra", "Meshy", "Tripo", "DemoAI"],
    "Text-Graphics":    ["ChatGPT", "Canva", "Ideogram", "Napin AI", "Playground AI", "Manus AI", "Julius AI"],
    "Image-Image":      ["OpenArt", "Leonardo AI", "Freepik", "Krea", "LullabyInk", "Google Nano Banana"],
    ...
    "Graphics-Animation": ["Hendra"],
    "Animation-Image":  [],
  };

  const currentToolNames = (activeInput && activeOutput)
        ...
        <div className="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 h-16 md:h-20 flex items-center justify-between">
          ...
          >
            <div className="relative w-40 h-10 sm:w-48 sm:h-12 md:w-56 md:h-14">
              <Image src="/logo.png?v=2" alt="Logo" fill className="object-contain" priority />
            </div>
          </div>
      ...
      <main className="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 py-8 flex flex-col gap-8">
        ...
        <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-6">
          ...
          <div className="flex flex-wrap items-center gap-3 lg:gap-4">
            {[
              { label: 'Streak', value: '12', suffix: 'Days', icon: Flame, color: 'text-orange-500', bg: 'bg-orange-50' },
              { label: 'Modules', value: '24', suffix: '', icon: CheckCircle, color: 'text-emerald-600', bg: 'bg-emerald-50' },
              { label: 'Accuracy', value: '94%', suffix: '', icon: Target, color: 'text-blue-600', bg: 'bg-blue-50' },
            ].map(({ label, value, suffix, icon: Icon, color, bg }) => (
              <div key={label} className="bg-white rounded-2xl px-5 py-4 border border-slate-200 shadow-sm flex items-center gap-4 min-w-[160px] hover:shadow-md hover:border-slate-300 transition-all duration-200">
                ...
                <div>
                  ...
                  <p className="text-2xl font-black text-slate-900 leading-none">
                    {value}{suffix && <span className="text-sm font-semibold text-slate-500 ml-1">{suffix}</span>}
                  </p>
                </div>
              </div>
            ))}
        ...
        <div className="flex flex-col lg:flex-row gap-6">
          ...
          <aside className="w-full lg:w-56 flex-shrink-0">
            <div className="bg-white rounded-2xl p-4 border border-slate-100 shadow-sm sticky top-24">
              <h3 className="text-[10px] font-bold text-slate-400 tracking-widest uppercase mb-3 px-1">Input Source</h3>
              <div className="space-y-1">
                {categories.map((item) => (
                  <button
                    key={item.name}
          ...
          <section className="flex-1 flex flex-col min-w-0">
            <div className="flex items-center justify-between mb-5">
              <div>
                <h2 className="text-xl font-bold text-slate-900">Tool Explorer</h2>
                <p className="text-sm text-slate-500 mt-0.5">Discover and master the best AI tools for your workflow.</p>
              </div>
            ...
            {activeInput && activeOutput && (
              <div className="flex items-center gap-2 mb-5 bg-white rounded-xl px-4 py-3 border border-slate-100 shadow-sm w-fit">
                <div className="flex items-center gap-1.5">
                  {(() => { const Icon = categories.find(c => c.name === activeInput)?.icon || Sparkles; return <Icon className="w-4 h-4 text-indigo-500" />; })()}
                  <span className="text-sm font-semibold text-slate-700">{activeInput}</span>
                </div>
                <ArrowRight className="w-4 h-4 text-slate-400" />
                <div className="flex items-center gap-1.5">
                  {(() => { const Icon = categories.find(c => c.name === activeOutput)?.icon || Sparkles; return <Icon className="w-4 h-4 text-purple-500" />; })()}
                  <span className="text-sm font-semibold text-slate-700">{activeOutput}</span>
                </div>
                <span className="text-xs text-slate-400 ml-2">· {currentToolNames.length} tools</span>
              </div>
            )}
            ...
            {!activeInput || !activeOutput ? (
              <div className="flex flex-col items-center justify-center py-24 bg-white rounded-2xl border border-slate-100 shadow-sm text-center px-4">
                ...
                <h3 className="text-lg font-bold text-slate-900 mb-2">Select Input &amp; Output</h3>
                <p className="text-sm text-slate-500 max-w-xs leading-relaxed">
                  Choose an input type on the left and an output type on the right to discover matching AI tools.
                </p>
              </div>
            ) : currentToolNames.length > 0 ? (
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="w-14 h-14 rounded-xl mb-3 flex items-center justify-center overflow-hidden bg-slate-50 border border-slate-100 group-hover:scale-105 transition-transform duration-200">
                        ...
                        <Sparkles className={`w-7 h-7 text-indigo-400 ${logoUrl ? 'hidden' : ''}`} />
                      </div>
              ...
              <div className="flex flex-col items-center justify-center py-16 bg-white rounded-2xl border border-slate-100 shadow-sm">
                ...
                <p className="text-sm text-slate-500">Try a different combination of input and output.</p>
              </div>
            )}
          </section>
          ...
          <aside className="w-full lg:w-56 flex-shrink-0">
            <div className="bg-white rounded-2xl p-4 border border-slate-100 shadow-sm sticky top-24">
              <h3 className="text-[10px] font-bold text-slate-400 tracking-widest uppercase mb-3 px-1">Target Output</h3>
              <div className="space-y-1">
                {categories.map((item) => (
                  <button
                    key={item.name}
