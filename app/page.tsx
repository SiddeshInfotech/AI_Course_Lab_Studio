...
import { EyeOff, Eye, ArrowRight, User, Lock, Shield, AlertCircle } from 'lucide-react';
          ...
          {!showAdmin ? (
            ...
            <form className="space-y-5" onSubmit={handleLogin}>
              <div className="space-y-1.5">
                <label className="text-[11px] font-bold text-slate-500 tracking-wider uppercase ml-1">Student ID</label>
              ...
              <div className="space-y-1.5">
                <label className="text-[11px] font-bold text-slate-500 tracking-wider uppercase ml-1">Password</label>
                <div className="relative">
                  <div className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400">
              ...
              <div className="pt-2">
                <button type="submit" className="w-full bg-[#4F46E5] hover:bg-indigo-700 text-white font-semibold rounded-xl px-4 py-4 flex items-center justify-center gap-2 transition-all hover:shadow-xl hover:shadow-indigo-500/30 active:scale-[0.98]">
            ...
            <form className="space-y-5" onSubmit={handleAdminLogin}>
              <div className="flex items-center gap-2 mb-1">
                ...
                <div>
                  <p className="text-sm font-bold text-slate-900 leading-none">Admin Login</p>
                  <p className="text-[10px] text-slate-400 leading-none mt-0.5">Restricted access</p>
                </div>
              </div>

              <div className="space-y-1.5">
                <label className="text-[11px] font-bold text-slate-500 tracking-wider uppercase ml-1">Admin ID</label>
                <div className="relative">
                  <div className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400">
                    <User className="w-5 h-5" />
                  </div>
              ...
              <div className="space-y-1.5">
                ...
                <div className="relative">
                  ...
                  />
                  <button type="button" onClick={() => setShowAdminPw(!showAdminPw)} className="absolute right-4 top-1/2 -translate-y-1/2 text-slate-400 hover:text-slate-600 transition-colors">
                    {showAdminPw ? <Eye className="w-5 h-5" /> : <EyeOff className="w-5 h-5" />}
                  </button>
                </div>
              </div>
              ...
              <div className="pt-2 space-y-3">
                <button type="submit" className="w-full bg-slate-900 hover:bg-slate-800 text-white font-semibold rounded-xl px-4 py-4 flex items-center justify-center gap-2 transition-all active:scale-[0.98]">
                  Sign In as Admin <ArrowRight className="w-5 h-5" />
                </button>
                <button
                  type="button"
                  onClick={() => { setShowAdmin(false); setAdminId(''); setAdminPassword(''); setAdminError(''); }}
