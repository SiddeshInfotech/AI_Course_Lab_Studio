...
export default function ProfileDropdown() {
  ...
  const displayId = user?.id ? `STU-${String(user.id).padStart(4, '0')}` : 'Guest';
    ...
    <div className="relative" ref={dropdownRef}>
      ...
      >
        ...
        <div className="hidden sm:flex flex-col items-start mr-1">
          ...
          <span className="text-[10px] font-semibold text-slate-500 uppercase tracking-wider">ID: {displayId}</span>
        </div>
        ...
        <div className="absolute right-0 mt-2 w-56 bg-white rounded-2xl shadow-xl border border-slate-100 py-2 z-50 animate-in fade-in slide-in-from-top-2 duration-200">
          <div className="px-4 py-3 border-b border-slate-100 mb-1 flex items-center gap-3">
            ...
            <div className="overflow-hidden">
              ...
              <p className="text-xs font-medium text-slate-500 truncate">{user?.email || user?.username}</p>
              {user?.isAdmin && (
