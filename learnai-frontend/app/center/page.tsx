...
export default function CenterAdminDashboardPage() {
  ...
  const centerStudents = useMemo(() => {
    ...
    return allStudents.filter((student) => student.centerId === centerAuth.centerId);
        ...
        <div className="rounded-2xl bg-white border border-slate-200 p-6 md:p-7 shadow-sm">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
              ...
              <h1 className="text-2xl md:text-3xl font-black mt-1 text-slate-900">
            ...
            <button
              ...
              className="inline-flex items-center justify-center gap-2 px-4 py-2.5 bg-slate-100 hover:bg-slate-200 border border-slate-200 rounded-lg text-sm font-semibold transition-colors text-slate-700"
            >
                ...
                <tbody>
                  {filteredStudents.map((student) => (
                    ...
                    >
                      ...
                      <td className="px-5 py-3 text-sm text-slate-700">{student.email}</td>
                      <td className="px-5 py-3 text-sm text-slate-700">{student.phoneNumber}</td>
                      <td className="px-5 py-3 text-sm text-slate-700">{student.courseClass}</td>
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm">
            <div className="px-5 py-4 border-b border-slate-100 flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-slate-500">Based on student distribution</p>
              </div>
                ...
                courseProgress.map((course) => (
                  <div key={course.courseClass}>
                    <div className="flex items-center justify-between mb-1.5">
                      <p className="text-xs font-semibold text-slate-700">{course.courseClass}</p>
